...
import net.minecraft.world.item.component.TooltipDisplay; import net.minecraft.world.level.Level; import net.minecraft.world.level.block.Block;
...
public class BoomboxItem extends BlockItem {
    ...
    private InteractionResult smartInteractHeld(Level level, Player player, InteractionHand usedHand) {
        ...
        if (!(boombox.getItem() instanceof BoomboxItem)) return InteractionResult.PASS;
    ...
    @Override public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay display, Consumer<Component> tooltip, TooltipFlag flag) {
        if (BoomboxData.hasDisc(stack)) { try { ItemStack disc = BoomboxData.readDisc(stack, context.registries()); Component discName = com.eyecrasher.lazoboombox.data.DiscNameUtil.get(disc, context.registries()); tooltip.accept(Component.translatable("lazoboombox.tooltip.contains_disc", discName).withStyle(ChatFormatting.AQUA)); } catch (Throwable t) { tooltip.accept(Component.translatable("lazoboombox.tooltip.contains_disc", Component.literal("?")).withStyle(ChatFormatting.AQUA)); } }
        else { tooltip.accept(Component.translatable("lazoboombox.tooltip.empty").withStyle(ChatFormatting.DARK_GRAY)); }
        BoomboxData.readOwner(stack).ifPresent(owner -> tooltip.accept(Component.translatable("lazoboombox.tooltip.owner", owner.toString()).withStyle(ChatFormatting.DARK_GRAY)));
    }
}
