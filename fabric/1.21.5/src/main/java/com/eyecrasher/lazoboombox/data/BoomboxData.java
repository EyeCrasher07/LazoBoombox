...
public final class BoomboxData {
    ...
    public static boolean hasDisc(ItemStack b) {
        ...
        return root.getCompound(ROOT_KEY).orElse(new CompoundTag()).contains(DISC_KEY);
    }
    ...
    public static void clearDisc(ItemStack b) {
        ...
        CompoundTag tag = root.getCompound(ROOT_KEY).orElse(new CompoundTag()); tag.remove(DISC_KEY);
    ...
    public static Optional<UUID> readOwner(ItemStack b) {
        ...
        CompoundTag tag = root.getCompound(ROOT_KEY).orElse(new CompoundTag());
        String s = tag.getString(OWNER_KEY).orElse(null); if (s == null || s.isBlank()) return Optional.empty();
